...
set val(x)		600
set val(y)		600
set opt(flowfin) 10;      #Time when CBR flows finish
set opt(simfin) 70;       #Time when simulation finishes
...
for {set i 1} {$i <= [expr $val(nn)]} {incr i} {
	...
	$ns_ connect $udp1($i) $null_($i)

	$ns_ at 1 "$cbr1($i) start"
	$ns_ at $opt(flowfin) "$cbr1($i) stop"
}
